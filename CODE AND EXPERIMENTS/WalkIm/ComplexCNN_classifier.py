...
## FUNCTIONS
from functions import create_ComplexCNN, preprocessing, metrics, saveModel, plot_loss_accuracy, saveConfMatrixClassReport
...
## MAIN  
  ...
  if type_encoder == "Grayscale":
    X_data = X_data.reshape((-1, 256, 128, 2)) # redefine the shape based on the DATASET: (n.examples, _, _, _) must have 4 dimension
  else:
  ...
  saveModel(dataset, model_cnn, 'Complex CNN', type_encoder)

  acc = plot_loss_accuracy(history, model_cnn, X_test, y_test, dataset, 'Complex CNN', type_encoder)

  conf_matrix, class_report = metrics(X_test, y_test, model_cnn)
  print('\n', conf_matrix, '\n', class_report)

  # save the results of classification model
  saveConfMatrixClassReport('Complex CNN', training_time, acc, conf_matrix, class_report, dataset, type_encoder)
