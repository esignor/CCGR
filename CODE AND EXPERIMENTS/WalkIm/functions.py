...
def create_ResNet50(model, shape, nb_classes):
    ...
    X = layers.Flatten()(X)
    X = layers.Dense(nb_classes, activation='softmax', name='fc' + str(nb_classes), kernel_initializer = glorot_uniform(seed=0))(X)
...
def plot_loss(history, dataset, model_net, type_encoder):
  directory='WalkIm ' + dataset + ' Results/'
...
def saveModel(dataset, model, net, type_encoder):
  directory='WalkIm ' + dataset + ' Models/'
  if not os.path.exists(directory):
    os.makedirs(directory)
